...
plt.title('D (разы) - Декартова система'); plt.grid(True); plt.legend()
plt.figure(figsize=(8, 5))
plt.plot(np.rad2deg(theta_an), D_an_db, 'b-', label='Аналитика')
plt.plot(cst_theta_deg, cst_D_db, 'r--', label='CST')
plt.title('D (dBi) - Декартова система'); plt.grid(True); plt.legend()
...
ax3.set_title('D (разы) - Полярная система', pad=15); ax3.legend(loc='lower right')
plt.figure(figsize=(6, 6))
...
ax4.set_title('D (dBi) - Полярная система', pad=15); ax4.legend(loc='lower right')
plt.show()
